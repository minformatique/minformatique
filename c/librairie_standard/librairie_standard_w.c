...
int main(int argc, char const *argv[]) {
  ...
  */
  ...
  /*
  ---> Windows seulement :
  Sous Windows la plupart des fonction ont été renommées pour casser la
  compatibilité avec UNIX et éviter que des programmes écrits pour Windows ne
  puissent être portés sous UNIX (je suppose, je ne vois pas trop d'autre
  explication).

  Par exemple fscanf a été renommée en fscanf_s, mais la fonction fait la même
  chose.
  */

  fscanf_s(stdin, "%s", big_buffer);
  ...
  /*
  ---> Windows seulement :
  Les chaînes de format semblent être interprétées avec de subtiles différences
  par rapport à UNIX. Ici les espaces autour du '+' sont obligatoires alors qu'ils
  ne l'étaient pas sous UNIX.
  */
  fscanf_s(stdin, "%d + %d", &first_int, &second_int);
  ...
  */
  ...
  /*
  ---> Windows seulement :
  scanf a été renommée en scanf_s, là encore je ne voit pas trop ce que cela
  apporte, surtout que la signature est la même que scanf.
  */
  scanf_s("%s", big_buffer);
  ...
  */
  ...
  /*
  ---> Windows seulement :
  fopen a été remplacée par fopen_s. Je pense qu'à ce point la convention de nommage
  des API de Windows doit vous sembler assez claire.

  IMPORTANT : Sous Windows, il faut toujours ouvrir les fichiers texte en mode "rb" ou
  "wb" pour les copier sinon vous allez voir apparaitre des problèmes assez étranges
  (vous pouvez changer cela dans ce fichier pour voir ce qui change).
  */
  fopen_s(&source, "librairie_standard.c", "rb");
  FILE *copy_source;
  fopen_s(&copy_source, "first_lines.txt", "wb");
  ...
  */
  ...
  fopen_s(&source, "librairie_standard.c", "rb");
  fopen_s(&copy_source, "copy_source.txt", "wb");
  ...
  */
  ...
  printf("Taille du fichier (routine) : %llu\n", get_file_size(source));
  /*
  ...
  tutoriel on se contentera d'une implémentation naïve, les améliorations
  ...
  */
  ...
  /*
  ---> Windows seulement :
  On peut utiliser aussi bien fread que fread_s.
  */
  fread_s(file_buffer, file_size, sizeof(char), file_size, source);
  ...
  */
  ...
  printf("%s", big_buffer);
  sscanf_s(big_buffer, "%d + %d", &first_int, &second_int);
  ...
  */
  ...
  printf("La commande %s s'est achevée avec le code %d\n", command,
         exit_status);
  if (exit_status == 9009) {
    printf("%s\n", "Il semble que python ne soit pas disponible.\n"
		    "Peut-être ne l'avez pas ajouté au PATH ?");
  }
...
int systemf(const char *format, ...) {
  ...
  */
  ...
  vsprintf_s(buffer, 1024 * sizeof(char), format, args);
