...
int main(int argc, char const *argv[]) {
  ...
  */
  ...
  /*
  ---> Windows uniquement :
  Windows ne respecte la spécification POSIX que dans les grandes lignes,
  et n'a pas vocation à être compatible avec UNIX. On a ici un exemple de
  cela. Dans la spécification UNIX le type size_t est défini comme le type
  que renvoie sizeof (en gros), or sous windows sizeof renvoie un "unsigned
  long long" alors que size_t est juste un "long".
  */
  size_t sizea = sizeof(a);

  /*
  ---> Windows uniquement :
  Les formats utilisés par printf ne sont pas les mêmes sous windows et linux.
  En particulier, sous linux (et la plupart des systèmes) on trouverait "%ld"
  au lieu de "%llu" dans le printf. Cela vient du fait que sizeof ne renvoie
  pas la même chose sous windows et linux.
  */
  printf("Taille d'un entier en mémoire %llu octets\n", sizea);
  ...
  */
  printf("Taille d'un flottant en mémoire %llu octets\n", sizeof(float));
