...
int main(int argc, char const *argv[]) {
  ...
  */
  ...
  /*
  ---> Windows seulement :
  J'aurais l'occasion d'y revenir longuement plus tard, mais la librairie
  standart de Windows n'est globalement pas compatible avec celle d'UNIX. On
  pourrait penser que cela serait parce que Windows et UNIX sont très différents
  en interne, mais non. La plupart des fois où Windows casse la compatibilité
  c'est juste pour éviter que l'on puisse porter un programme sous Windows vers
  un autre OS.

  Un exemple assez flagrant ci dessous : UNIX utilise une fonction nommée strncpy
  pour copier des chaines de caractères, mais Windows choisit de définir strncpy_s
  qui fait la même chose, mais avec des arguments et des types de retour différent.
  
  J'ai mis en commentaire la version UNIX de la même ligne de code, comme vous
  pourrez la différence est vraiment esthétique.

  Pour la défense de Windows, ces problèmes de compatibilités sont issus d'une
  autre époque et il ne faut pas y voir le reflet de la politique actuelle de
  Microsoft, plutôt de sa politique dans les années 90 - 2000.

  La version UNIX :
  strncpy(my_machine.identifiant, "absurde", 8 * sizeof(char));
  */
  strncpy_s(my_machine.identifiant, 8 * sizeof(char), "absurde", _TRUNCATE);
  ...
  */
  ...
  strncpy_s(machine.identifiant, 8 * sizeof(char), "absurde", _TRUNCATE);
